...
const PAIN_POINT_RESPONSES: Record<string, string> = {
  ...
  "Mes notes sont illisibles après coup":
    "Memoflash transforme automatiquement ta voix en fiche propre et lisible, plus de notes illisibles.",
};
  ...
  return (
    <div className="dot-grid relative isolate flex min-h-screen flex-col items-center overflow-hidden bg-[#0b1120] px-4 py-16">
      ...
      {!isDone ? (
        ...
        <div className="fiche-enter w-full max-w-md rounded-lg border border-[#232d45] bg-[#141b2e] p-8 text-center">
          <span className="mb-5 inline-block rounded-full border border-[#2a3552] px-3 py-1 text-xs font-medium text-[#8b97b0]">
            On a compris ta situation
          </span>
          <h1 className="mb-4 text-balance text-xl font-bold text-[#e7ecf5]">
            Tu veux {GOAL_PHRASES[goal] ?? "gagner du temps sur tes études"}, et ta plus
            grosse galère c&apos;est :{" "}
            <span className="text-[#38bdf8]">{painPoint}</span>
          </h1>
          <p className="mb-8 text-[#8b97b0]">
            Avec Memoflash,{" "}
